  ...
  // ── CHANGE PASSWORD ────────────────────────────────────────────────────────
  let changeStep = 'master'; // master → new → confirm
  ...
  function addPlayer() {
    ...
    input.value = ''; renderPlayerList(); saveData(); input.focus();
  }
  document.getElementById('new-player-input').addEventListener('keydown', e => { if (e.key === 'Enter') addPlayer(); });
  function removePlayer(id) { players = players.filter(p => p.id !== id); renderPlayerList(); saveData(); }
  function renderPlayerList() {
    const list = document.getElementById('player-list');
    if (!players.length) { list.innerHTML = '<div class="empty-msg">Nenhum jogador. Adicione acima ☝️</div>'; return; }
    list.innerHTML = players.map((p, i) => `
      <div class="player-item" draggable="true" data-index="${i}"
